...
class ClinicalTrial(TrialRegistrationDataSetModel):
    ...
    def create_fossil(self):
        ...
        fossil.set_indexer(key='hc_freetext', value="%s%s" % (self.hc_freetext, '|'.join([trans.hc_freetext for trans in self.translations.all()])))        
        fossil.set_indexer(key='i_freetext', value="%s%s" % (self.i_freetext, '|'.join([trans.i_freetext for trans in self.translations.all()])))
